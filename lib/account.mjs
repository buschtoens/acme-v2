...
import wrapURL from './decorators/wrap-url';
...
export default class Account {
  ...
  constructor(
    acme,
    { key, accountURL, contact = [], termsOfServiceAgreed = false }
  ) {
   ...
   * @param {ACMEV2Client} acme
   * @param {jose.JWK.key} key
   * @param {boolean} loadInformation Wether or not the accound information
   *   should be loaded before returning the account.
   * @return {Account}
   * @throws {AccountDoesNotExistError}
   */
  static async findAccountByKey(acme, key, loadInformation = true) {
    const account = new Account(acme, {
      key,
      accountURL: await Account.findAccountURLByKey(acme, key)
    });

    if (loadInformation) await account.load();
   ...
   * @param {boolean} options.kid If `true`, send the `accountURL` as `kid`.
   * @param {boolean} options.jwk If `true`, send the `key` as `jwk`.
   * @param {object} options.jwsHeaders
   */
  async signedRequest(
    url,
    payload,
    { kid = false, jwk = false, jwsHeaders, ...options } = {}
  ) {
    if (kid && jwk) throw new Error('kid and jwk are mutually exclusive.');

    return this.acme.signedRequest(this.key, url, payload, {
      jwsHeaders: {
        kid: kid ? this.accountURL : null,
        jwk: jwk ? this.key.toJSON() : null,
        ...jwsHeaders
      },
      ...options
    });
  ...
  async updateKey(newKey) {
     ...
     */
    this.signedRequest(url, keyChange, { jwk: true });
  ...
  /**
   * Creates and submits a new order for this account.
   * @param {Object} details
   * @return {Order}
   */
  async createOrder(details) {
    return Order.create(this, details);
  }
}
